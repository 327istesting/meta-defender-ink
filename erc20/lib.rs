...
mod erc20 {
    ...
    impl Erc20 {
        ...
        pub fn transfer(&mut self, to: AccountId, value: Balance) -> Result<()> {
            ...
            // ink::env::debug_println!("从这个account发起转账{:?}", from);
            // ink::env::debug_println!("balance is {}", self.balance_of(from));
        ...
        pub fn transfer_from(
            ...
            ink::env::debug_println!("从这个account发起转账{:?}, 从这个地址转出去{:?}", caller, from);
            self.transfer_from_to(&from, &to, value)?;
            self.allowances
